...
HRESULT Decompile(IDxcBlob *pProgram, raw_string_ostream &Stream) {
  try {
  ...
  auto EmitMetaData = [&llvmContext](DxilModule &dxilModule,
                                     raw_string_ostream &Stream) -> HRESULT {
    const llvm::Module *pModule = dxilModule.GetModule();
    const llvm::NamedMDNode* meta = pModule->getNamedMetadata("dx.shaderModel");
    meta->print(Stream);
    return S_OK;
  };

  auto EmitEntrySig = [&llvmContext](DxilModule &dxilModule,
                                     raw_string_ostream &Stream) -> HRESULT {
    const llvm::Function *fn = dxilModule.GetEntryFunction();
   // const DxilEntrySignature &sig = dxilModule.GetDxilEntrySignature(fn);

    if (!dxilModule.HasDxilEntryProps(fn))
      return DXC_E_GENERAL_INTERNAL_ERROR;

    DxilEntryProps &props = dxilModule.GetDxilEntryProps(fn);

    if (!props.props.IsCS())
      return DXC_E_GENERAL_INTERNAL_ERROR;

    Stream << "[numthreads(" << props.props.ShaderProps.CS.numThreads[0] << "," << props.props.ShaderProps.CS.numThreads[1] << "," << props.props.ShaderProps.CS.numThreads[2] << ")]\n";
    Stream << "void " << dxilModule.GetEntryFunctionName() << "()\n";

   // if (props.sig.InputSignature.GetSigPointKind() != SigPointKind::CSIn)
   //     return DXC_E_GENERAL_INTERNAL_ERROR;
   //
   // auto& elems = props.sig.InputSignature.GetElements();
   //
  ...
  auto EmitEntryBody = [&llvmContext](DxilModule &dxilModule,
                                      raw_string_ostream &Stream)->HRESULT {
    hlsl::OP op(llvmContext, dxilModule.GetModule());

    llvm::Function *fn = dxilModule.GetEntryFunction();

    do
    {
      Stream << "{\n";

      const llvm::ValueSymbolTable &sym = fn->getValueSymbolTable();
      for (const auto &it : sym) {
        Stream << it.getKey() << " | ";
      }

      for (const llvm::BasicBlock &block : fn->getBasicBlockList()) {
        Stream << "\n";
        Stream << "\n";
        for (const llvm::Instruction &inst : block.getInstList()) {

          std::string instName = inst.getOpcodeName();

          if (hlsl::OP::IsDxilOpFuncCallInst(&inst)) {
            DXIL::OpCode opCode = hlsl::OP::GetDxilOpFuncCallInst(&inst);

            Stream << op.GetOpCodeName(opCode) << " // " << instName;
            ...
            for (auto it = inst.op_begin(); it != inst.op_end(); ++it) {
              const llvm::Value *value = it->get();
              if (value->hasName())
                Stream << value->getValueName() << ",";

              const llvm::Type *valueType = value->getType();
              Stream << valueType->getTypeID() << ",";
            }
          }
          Stream << "\n";
        }
